...
void vcon_exit(vcon_handle_t *sh)
{
  ...
  if(sh->fh) {
    FreeDosObject(DOS_FILEHANDLE, sh->fh);
  }
...
static ULONG handle_pkt(vcon_handle_t *sh, struct Message *msg)
{
  ...
  switch(type) {
    ...
    case ACTION_CHANGE_SIGNAL: {
      ...
      Printf("CHANGE_SIGNAL: fh=%lx msg_port=%lx\n", fh, msg_port);
      /* return old port */
      res1 = DOSTRUE;
    ...
    case ACTION_WRITE: {
      ...
      Printf("WRITE: fh=%lx buf=%lx size=%ld %s\n", fh, buf, size, buf);
    ...
    case ACTION_READ: {
      APTR buf = (APTR)pkt->dp_Arg2;
      LONG size = pkt->dp_Arg3;
      Printf("READ: fh=%lx buf=%lx size=%ld\n", fh, buf, size);
      /* add to rw list and do not reply now */
      do_reply = FALSE;
      AddTail(&sh->rw_list, (struct Node *)msg);
    ...
    case ACTION_SEEK: {
      ...
      Printf("SEEK: offset=%ld mode=%ld\n", offset, mode);
      res1 = DOSFALSE;
      res2 = ERROR_OBJECT_WRONG_TYPE;;
      break;
    }
    default:
      Printf("Unknown PKT: %ld\n", type);
      res1 = DOSFALSE;
      res2 = ERROR_ACTION_NOT_KNOWN;
      break;
  }
  if(do_reply) {
      Printf("do-reply: res1=%ld res2=%ld\n", res1, res2);
...
{
  if(sh->cur_flags & VCON_HANDLE_READ) {
    if(sh->head_pkt != NULL) {
...
LONG vcon_read(vcon_handle_t *sh, vcon_io_func_t read_func, APTR user_data)
{
  ...
  /* callback read */
  LONG result = read_func(buf, size, user_data);
...
{
  if(sh->cur_flags & VCON_HANDLE_WRITE) {
    if(sh->head_pkt != NULL) {
      struct DosPacket *pkt = sh->head_pkt;
      LONG size = pkt->dp_Arg3;
      return size;
...
LONG vcon_write(vcon_handle_t *sh, vcon_io_func_t write_func, APTR user_data)
{
  ...
  /* callback read */
  LONG result = write_func(buf, size, user_data);
